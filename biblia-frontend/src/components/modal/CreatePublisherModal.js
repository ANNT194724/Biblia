...
const CreatePublisherModal = (props) => {
  ...
  const handlePhotoUpload = (event) => {
    ...
    if (file) {
      ...
      const coverRef = storageRef.child(`publishers/${file.name}`);
  ...
  const createPublisher = (event) => {
    event.preventDefault();
    setLoading(true);
    setError(null);

    const apiEndpoint = `${BASE_URL}/publisher`;
  ...
  return (
    <CModal backdrop="static" visible={props.visible} onClose={props.onHide} size="lg">
      <CModalHeader>
        <CModalTitle>Thêm nhà xuất bản</CModalTitle>
      </CModalHeader>
      <CModalBody>
        {logo_url && (
      ...
      <CModalFooter>
        ...
        <CButton color="primary" onClick={createPublisher} disabled={loading}>
          Thêm nhà xuất bản
        </CButton>
      </CModalFooter>
    </CModal>
...
CreatePublisherModal.propsType = {
